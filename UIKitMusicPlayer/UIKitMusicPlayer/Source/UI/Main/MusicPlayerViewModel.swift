...
public final class MusicPlayerViewModel: ObservableObject {
    ...
    private(set) var walkman: MusicPlayerProtocol?
    ...
    public init() {
        ...
        musicDataSubject
            ...
            }, receiveValue: { [weak self] walkman in
                self?.walkman = walkman

                self?.musicInteractor?.updateEndTime(walkman.endTime)
                self?.bindMusicPlayer(walkman)
            })
            .store(in: &cancellables)
...
private extension MusicPlayerViewModel {
    func bindMusicPlayer(_ walkman: MusicPlayerProtocol) {
        walkman.currentTimeDidUpdatePublisher
            .compactMap { $0 }
        ...
        walkman.didFinishPlayingPublisher
            .compactMap { $0 }
            .filter { $0 }
            .sink { [weak self] _ in
        ...
        walkman.decodeErrorDidOccurPublisher
            .compactMap { $0 }
